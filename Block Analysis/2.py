...
def main():
    ...
    file_path = r"C:\Users\123456\Desktop\pachong\n2渗透数目.xlsx"
    try:
    ...
    # 3. 计算不同分组大小的标准误差
    max_group_size = min(n // 2, 99)  # 最大分组大小为n//2或50中的较小值
    ...
    if len(valid_sizes_ns) <= 20:  # 如果点不多，添加所有标签
        ...
        for i in range(min(3, len(valid_sizes_ns))):
            plt.annotate(f'{valid_errors[i]:.4f}', (valid_sizes_ns[i], valid_errors[i]),
    ...
    # 保存到Excel
    excel_output_path = r"C:\Users\123456\Desktop\pachong\n2渗透数目标准误差.xlsx"
    result_df.to_excel(excel_output_path, index=False)
    print(f"\n结果已保存到 '{excel_output_path}'")

    # 保存图表
    chart_output_path = r"C:\Users\123456\Desktop\pachong\n2渗透数目.jpg"
